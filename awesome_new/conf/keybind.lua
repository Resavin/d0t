...
cyclefocus.display_notifications = true
...
local rmbflag = false
local user = "byakuya"
local date = os.date("%Y-%m-%d") -- used for going to today's screenshot folder
...
awful.keyboard.append_global_keybindings({
	...
	-- 	{ modkey, "Shift" },
	-- 	"/",
	-- 	hotkeys_popup.show_help({ show_awesome_keys = false }),
	-- 	{ description = "show help", group = "awesome" }
	-- ),
	-- https://github.com/awesomeWM/awesome/blob/master/lib/awful/hotkeys_popup/widget.lua
	-- Надо в функцию походу запихнуть было, я в принципе думал так сделать
	awful.key(
		{ "Shift", modkey },
		"/",
		function()
			hotkeys_popup.show_help(nil, nil, { show_awesome_keys = true })
		end,
		nil, -- Хз, что это делает
		{
			description = "show help for current app",
			group = "HELP",
		}
	),
	...
	-- awful.key({ modkey }, "x", DOESN'T work without a prompt
	--           function ()
	--               awful.prompt.run {
	--                 prompt       = "Run Lua code: ",
	...
	awful.key({ modkey }, "d", function()
		awful.spawn.with_shell("rofi -i -show drun -modi drun -show-icons")
	end, { description = "Drun prompt", group = "launcher" }),
	...
	awful.key({ modkey, "Control" }, "e", function()
		awful.spawn("pcmanfm")
	end, { description = "open pcman", group = "launcher" }),
	awful.key({ modkey }, "e", function()
		awful.spawn("nautilus --new-window")
	end, { description = "open nautilus", group = "launcher" }),

	awful.key({ modkey, "Shift" }, "s", function()
		-- awful.util.spawn_with_shell("/home/".. user .. "/d0t/customScripts/screenshot") end,
		awful.util.spawn_with_shell("screenshot")
	end, { description = "screenshot selected area", group = "scripts" }),
	...
	end, { description = "offnutEcran", group = "scripts" }),

	awful.key(
		{ modkey, "Shift" },
		"a",
		function()
			awful.util.spawn_with_shell("rofi-wifi")
		end,
	...
	awful.key(
		...
		"b",
		function()
			awful.util.spawn_with_shell("rofi-bluetooth")
		end,
		-- awful.spawn.easy_async_with_shell("curl -s eth0.me | dmenu") end,
		{ description = "bluetooth menu", group = "scripts" }
	),

	awful.key({ modkey, "Shift" }, "x", function()
		-- awful.util.spawn("/home/mehrunes/customScripts/lock") end,
		awful.util.spawn_with_shell("i3lock-fancy -f Iosevka")
	end, { description = "lock screen", group = "scripts" }),
	...
	end, { description = "chromium run or raise", group = "launcher" }),
})
...
awful.keyboard.append_global_keybindings({
	...
	awful.key({}, "XF86AudioMicMute", function()
		...
		awesome.emit_signal("signal::mic")
	end, { description = "mute mic", group = "laptop" }),
...
client.connect_signal("request::default_keybindings", function()
	awful.keyboard.append_client_keybindings({
		...
		end, { description = "Decrease size in floating", group = "client" }),
		awful.key({ modkey, "Shift" }, "Up", function(c)
			c:relative_move(-20, -20, 40, 40)
		end, { description = "Increase size in floating", group = "client" }),
		-- end
	})
end)

local lain = require("lain")
-- local quake = lain.util.quake({settings = function() app = "alacritty" end}) doesn't change the terminal
...
awful.keyboard.append_global_keybindings({
	...
	awful.key({ modkey }, "r", function()
		quake:toggle()
	end, { description = "quake menu", group = "customise" }),
})

awful.keyboard.append_global_keybindings({
	awful.key({ modkey, "Shift" }, "r", function()
		awful.spawn.with_shell("xrandr --output eDP --auto")
	end, { description = "turn on laptop display", group = "customise" }),
})
